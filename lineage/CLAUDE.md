...
- **03 パイプラインの構造**：`lnge_render_dynamic_sql`（8 プレースホルダ / 9 パラメータ）で
  ...
  全 DECLARE は最初の文より前）。**`[A]` 内はさらに「DECLARE を1ブロックに固める
  （各行に短いインラインコメント）＋その下に `Variable notes:` として変数名ごとの
  詳細説明」の形に統一**（01/03/04/06/07/08/09 すべて）。設定すべき変数が一覧で
  見渡せるようにするためで、詳細は下のノートを参照する。挙動はコメント再配置のみで不変。
