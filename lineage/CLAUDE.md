...
- **03 パイプラインの構造**：`render_dynamic_sql`（8 プレースホルダ / 9 パラメータ）で
  ...
  さらに 03 の DECLARE 群は `[A] 必須設定（デプロイ/リージョンごと）`→
  `[B] 動作オプション`→`[C] 派生/内部（編集不可）`の3段に整理。新リージョン
  立ち上げ時に触るのは冒頭の `SET @@location` と `[A]` だけ。並べ替えとコメント
  のみで挙動不変（各変数は1回ずつ宣言・master は role より前・全 DECLARE は
  最初の SET より前）。
...
- バンドル: `sha256 = 9c22647c248ac02f809220b8e0d3b185543d3cbc77e234c9b9f27a3c8e52a58a`、`445590` bytes
